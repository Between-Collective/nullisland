...
const aisSentinels: Transform = (ctx) => {
  ...
  note(
    ctx,
    `AIS not-available sentinels are in band: ${group(moved)} positions are 91/181, ` +
...
const centroidFallback: Transform = (ctx) => {
  ...
  note(
    ctx,
    `${group(count)} feature(s) that could not be placed sit on ${where} — ` +
...
const bidstreamRounding: Transform = (ctx) => {
  ...
  note(
    ctx,
    `${group(count)} position(s) truncated to two decimal places by the exchange, ` +
...
const lon0to360: Transform = (ctx) => {
  ...
  note(
    ctx,
    `${group(count)} longitude(s) rewritten into the 0–360 domain that global grids use. ` +
...
const stationaryDrift: Transform = (ctx) => {
  ...
  note(
    ctx,
    `${group(count)} stationary feature(s) wander about 15 m a fix with the engine off. ` +
...
const sliverGaps: Transform = (ctx) => {
  ...
  note(
    ctx,
    `${group(count)} polygon(s) pulled back from their shared edges by a few centimetres. ` +
...
const overlappingAreas: Transform = (ctx) => {
  ...
  note(
    ctx,
    `${group(extras.length)} overlapping area(s) added: each one nests the smaller one ` +
...
const trackBreaks: Transform = (ctx) => {
  ...
  if (gapped) {
    note(
      ctx,
      `${group(gapped)} track(s) have a receiver gap: a run of fixes is missing and the ` +
  ...
  if (shuffled) {
    note(
      ctx,
      `${group(shuffled)} track(s) have vertices in arrival order rather than time order, ` +
...
const envelopeFootprints: Transform = (ctx) => {
  ...
  note(
    ctx,
    `${group(count)} outline(s) replaced by their axis-aligned envelope, claiming ground ` +
...
const danglingNodes: Transform = (ctx) => {
  ...
  note(
    ctx,
    `${group(count)} line(s) undershoot or overshoot the junction they should meet. ` +
...
const unitMixture: Transform = (ctx) => {
  ...
  note(
    ctx,
    `${group(count)} row(s) report ${swap.field} in ${swap.to} while the rest are in ` +
...
const sentinelValues: Transform = (ctx) => {
  ...
  note(
    ctx,
    `${group(count)} row(s) carry ${chosen.value} in ${chosen.field}, meaning ` +
...
const leadingZeros: Transform = (ctx) => {
  ...
  note(
    ctx,
    `${group(count)} zero-padded code(s) in ${[...touched].join(", ")} came back as numbers, ` +
...
const paddedValues: Transform = (ctx) => {
  ...
  note(
    ctx,
    `${group(count)} value(s) in ${[...names].join(", ")} are padded out to the wire width. ` +
...
const unstableIdentity: Transform = (ctx) => {
  ...
  note(
    ...
    `${identity.rotating} is the equipment, not the thing it is bolted to: ` +
      `${group(rotated)} row(s) carry a fresh one and ${group(shared)} share a single ` +
...
const keyFormatDrift: Transform = (ctx) => {
  ...
  note(
    ctx,
    `${field} is written ${4} different ways across ${group(count)} row(s) — separators ` +
...
const taxonomyDrift: Transform = (ctx) => {
  ...
  note(
    ctx,
    `${chosen.field} switches code list halfway through the file: ${group(count)} row(s) use ` +
...
const unusableLabels: Transform = (ctx) => {
  ...
  note(
    ctx,
    `${chosen.field} is not a code list: ${group(count)} row(s) hold values like ` +
...
const cachedArea: Transform = (ctx) => {
  ...
  note(
    ...
    `${field} was cached from a projected copy of this data and never recomputed: ` +
      `${group(count)} row(s) disagree with the geometry beside them.`,
  );
};
...
const snowflakePrecision: Transform = (ctx) => {
  ...
  note(
    ctx,
    `${group(count)} 64-bit id(s) were parsed as JSON numbers, so the last few digits are ` +
...
const pastMidnightTimes: Transform = (ctx) => {
  ...
  note(
    ctx,
    `${group(count)} arrival time(s) are past 24:00:00 — the GTFS way of saying "after ` +
...
const defaultDatetimes: Transform = (ctx) => {
  ...
  note(
    ctx,
    `${group(count)} unknown ${field} value(s) default to midnight on the 1st of January. ` +
...
const waterOnlyTract: Transform = (ctx) => {
  ...
  note(
    ctx,
    `${group(count)} polygon(s) report an ALAND of 0 while still carrying a population. ` +
...
const supersededRecords: Transform = (ctx) => {
  ...
  note(
    ctx,
    `${group(extras.length)} superseded record(s) are still in the file beside the ones that ` +
...
const stackedRecords: Transform = (ctx) => {
  ...
  note(
    ctx,
    `${group(extras.length)} record(s) share an outline with another feature — the condo ` +
...
const vintageMismatch: Transform = (ctx) => {
  ...
  note(
    ctx,
    `${group(count)} row(s) are keyed to a different boundary vintage, in the same column, ` +
...
export function unquotedCommas(text: string): { text: string; notes: string[] } {
  ...
  return {
    ...
    notes: count
      ? [
          `${group(count)} field(s) containing commas were written unquoted, so every column ` +
